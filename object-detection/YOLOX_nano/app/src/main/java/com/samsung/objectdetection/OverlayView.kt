...
class OverlayView(
    ...
    private fun getScaledCoordinates(boundingBox: RectF): RectF {
        ...
        val squareSize = min(vw, vh)
        val dx = (vw - squareSize) / 2f
        val dy = (vh - squareSize) / 2f

        val left   = dx + boundingBox.left * squareSize
        val top    = dy + boundingBox.top * squareSize
        val right  = dx + boundingBox.right * squareSize
        val bottom = dy + boundingBox.bottom * squareSize
